 ...
 */
@objc public protocol YYTextKeyboardObserver: NSObjectProtocol {
    @objc optional func keyboardChanged(with transition: YYTextKeyboardTransition)
}
 ...
 */
public class YYTextKeyboardTransition: NSObject {
 ...
 */
public class YYTextKeyboardManager: NSObject {
    ...
    @objc public var keyboardWindow: UIWindow? {
        ...
        let window: UIWindow? = YYLabel.keyWindow
        if (_getKeyboardView(from: window) != nil) {
            return window
        }
    ...
    @objc(defaultManager)
    public static let `default` = YYTextKeyboardManager()
     ...
     */
    @objc(addObserver:)
    public func add(observer: YYTextKeyboardObserver?) {
     ...
     */
    @objc(removeObserver:)
    public func remove(observer: YYTextKeyboardObserver?) {
        guard let observer = observer else {
            return
        }
    ...
    private var observers: NSHashTable<YYTextKeyboardObserver>
    ...
    @objc private func _notifyAllObservers() {
        ...
        let trans = YYTextKeyboardTransition()
        // from
        ...
        if !trans.toFrame.equalTo(fromFrame) {
            ...
            for (_, observer) in observers.objectEnumerator().enumerated() {
                guard let o = observer as? YYTextKeyboardObserver else {
                    return
                }
                if o.responds(to: #selector(YYTextKeyboardObserver.keyboardChanged(with:))) {
    ...
    public func convert(_ rect: CGRect, to view: UIView?) -> CGRect {
        ...
        var mainWindow: UIWindow? = YYLabel.keyWindow
        if mainWindow == nil {
