 ...
 */
@objc public protocol YYTextDebugTarget: NSObjectProtocol {
    /**
     ...
     */
    var debugOption: YYTextDebugOption? { get set }
}


fileprivate var sharedDebugLock = DispatchSemaphore(value: 1)

/// A List Of YYTextDebugOption (Unsafe Unretain)
fileprivate var sharedDebugTargets = NSPointerArray()

@objc public class YYTextDebugOption: NSObject, NSCopying {
    
    private static let _shared = YYTextDebugOption()
    private static var sharedOption: YYTextDebugOption?
    
    @objc public static var shared: YYTextDebugOption {
        get {
     ...
     */
    @objc(addDebugTarget:)
    public class func add(_ target: YYTextDebugTarget?) {
    ...
    public class func remove(_ target: YYTextDebugTarget?) {
     ...
     */
    @objc public class func sharedDebugOption() -> YYTextDebugOption? {
        
        sharedDebugLock.wait()
        let op = YYTextDebugOption.shared
        sharedDebugLock.signal()
     ...
     */
    @objc public class func setSharedDebugOption(_ option: YYTextDebugOption) {
        assert(Thread.isMainThread, "This method must be called on the main thread")
        
        sharedDebugLock.wait()
        YYTextDebugOption.shared = option
        for target in sharedDebugTargets.allObjects {
            (target as? YYTextDebugTarget)?.debugOption = YYTextDebugOption.shared
        }
        sharedDebugLock.signal()
    }
}
