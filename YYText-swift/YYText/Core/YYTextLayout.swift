...
@inline(__always) private func textClipCGSize(_ size: CGSize) -> CGSize {
    var size = size
    if size.width > YYTextContainer.textContainerMaxSize.width {
        size.width = YYTextContainer.textContainerMaxSize.width
    }
    if size.height > YYTextContainer.textContainerMaxSize.height {
...
/**
 ...
 └─────────────────────────────┘
 */
public class YYTextContainer: NSObject, NSCoding, NSCopying, NSSecureCoding {
    ...
    @objc public var size: CGSize {
        set {
            if readonly {
                fatalError("Cannot change the property of the 'container' in 'YYTextLayout'.")
            }
            lock.wait()
            if _path == nil {
    ...
    private var _truncationType = YYTextTruncationType.none
    /// The line truncation type, default is none.
    @objc public var truncationType: YYTextTruncationType {
        set {
            if readonly {
                fatalError("Cannot change the property of the 'container' in 'YYTextLayout'.")
            }
    ...
    private weak var _linePositionModifier: YYTextLinePositionModifier?
    ...
    @objc public weak var linePositionModifier: YYTextLinePositionModifier? {
        set {
            ...
            lock.wait()
            _linePositionModifier = _linePositionModifier?.copy() as? YYTextLinePositionModifier
            lock.signal()
        }
        get {
            lock.wait()
    ...
    ///< used only in YYTextLayout.implementation
    ...
    public class func container(with size: CGSize) -> YYTextContainer {
    ...
    public class func container(with size: CGSize, insets: UIEdgeInsets) -> YYTextContainer {
        let one = YYTextContainer.init()
    ...
    public class func container(with path: UIBezierPath?) -> YYTextContainer {
        let one = YYTextContainer.init()
        one.path = path
        return one
    }
    ...
    required convenience public init?(coder aDecoder: NSCoder) {
        ...
        _truncationType = YYTextTruncationType(rawValue: aDecoder.decodeInteger(forKey: "truncationType"))!
        _truncationToken = aDecoder.decodeObject(forKey: "truncationToken") as? NSAttributedString
        _linePositionModifier = aDecoder.decodeObject(forKey: "linePositionModifier") as? YYTextLinePositionModifier
    }
 ...
 */
@objc public protocol YYTextLinePositionModifier: NSObjectProtocol, NSCopying {
    /**
 ...
 */
public class YYTextLinePositionSimpleModifier: NSObject, YYTextLinePositionModifier {
    
    ///< The fixed line height (distance between two baseline).
    @objc public var fixedLineHeight: CGFloat = 0
    
    public func modifyLines(_ lines: [YYTextLine]?, fromText text: NSAttributedString?, in container: YYTextContainer?) {
    ...
    public func copy(with zone: NSZone? = nil) -> Any {
        let one = YYTextLinePositionSimpleModifier()
...
/**
 ...
 └──────────────────────────┘
 */
public class YYTextLayout: NSObject, NSCoding, NSCopying {
    ...
    ///< The text container
    @objc public private(set) lazy var container = YYTextContainer()
    ///< The full text
    ...
    @objc public private(set) lazy var frame: CTFrame = {
        ...
        let ctFrame = CTFramesetterCreateFrame(ctSetter, YYTextUtilities.textCFRange(from: NSRange(location: 0, length: 0)), UIBezierPath().cgPath, [:] as CFDictionary)
        return ctFrame
    }()
    ///< Array of `YYTextLine`, no truncated
    @objc public private(set) lazy var lines: [YYTextLine] = []
    ///< YYTextLine with truncated token, or nil
    @objc public private(set) var truncatedLine: YYTextLine?
    ///< Array of `YYTextAttachment`
    @objc public private(set) var attachments: [YYTextAttachment]?
    ...
    ///< top-left origin
    private var lineRowsEdge: UnsafeMutablePointer<YYRowEdge>?
    ...
    private convenience init(container: YYTextContainer) {
        self.init()
    ...
    public convenience init?(containerSize: CGSize, text: NSAttributedString?) {
        let container = YYTextContainer.container(with: containerSize)
    ...
    public convenience init?(container: YYTextContainer?, text: NSAttributedString?) {
    ...
    public convenience init?(container: YYTextContainer?, text: NSAttributedString?, range: NSRange) {
        
        guard let t = text?.mutableCopy() as? NSMutableAttributedString, let c = container?.copy() as? YYTextContainer else {
            return nil
        }
        ...
        var lineCount: Int = 0
        var lines_ = [YYTextLine]()
        var attachments_: [YYTextAttachment]? = nil
        ...
        var truncatedLine_: YYTextLine? = nil
        var lineRowsEdge_: UnsafeMutablePointer<YYRowEdge>? = nil
        ...
        if needFixJoinedEmojiBug {
            (text as? NSMutableAttributedString)?.yy_setClearColorToJoinedEmoji()
        }
        ...
        if c.path == nil && (c.exclusionPaths?.count ?? 0) == 0 {
            ...
            if needFixLayoutSizeBug {
                ...
                if c.isVerticalForm {
                    rect.size.width = YYTextContainer.textContainerMaxSize.width
                } else {
                    rect.size.height = YYTextContainer.textContainerMaxSize.height
                }
            }
        ...
        let ctFrame = CTFramesetterCreateFrame(ctSetter, YYTextUtilities.textCFRange(from: range), cgPath, frameAttrs as CFDictionary)
        ...
        for i in 0..<lineCount {
            ...
            let line = YYTextLine.lineWith(ctLine: ctLine, position: position, vertical: isVerticalForm)
        ...
        if rowCount_ > 0 {
            ...
            lineRowsEdge_ = UnsafeMutablePointer<YYRowEdge>.allocate(capacity: rowCount_)
            ...
            while i < maxCount {
                ...
                if line.row != lastRowIdx {
                    if lastRowIdx >= 0 {
                        lineRowsEdge_![lastRowIdx] = YYRowEdge(head: lastHead, foot: lastFoot)
                    }
                    lastRowIdx = line.row
            ...
            for i in 1..<rowCount_ {
                let v0: YYRowEdge = lineRowsEdge_![i - 1]
                let v1: YYRowEdge = lineRowsEdge_![i]
        ...
        if true {
            ...
            if (c.path != nil) {
                ...
                rect = rect.inset(by: YYTextUtilities.textUIEdgeInsetsInvert(c.insets))
            }
            rect = rect.standardized
        ...
        var visibleRange_ = YYTextUtilities.textNSRange(from: CTFrameGetVisibleStringRange(ctFrame))
        ...
        if needTruncation {
            ...
            if c.truncationType != YYTextTruncationType.none {
                ...
                if (c.truncationToken != nil) {
                    ...
                    truncationToken = NSAttributedString(string: YYTextAttribute.textTruncationToken, attributes: attrs)
                    truncationTokenLine = CTLineCreateWithAttributedString(truncationToken! as CFAttributedString)
                }
                
                if (truncationTokenLine != nil) {
                    var type: CTLineTruncationType = .end
                    if c.truncationType == YYTextTruncationType.start {
                        type = .start
                    } else if c.truncationType == YYTextTruncationType.middle {
                        type = .middle
                    }
                    ...
                    if let ctTruncatedLine = CTLineCreateTruncatedLine(ctLastLineExtend, Double(truncatedWidth), type, truncationTokenLine) {
                        truncatedLine_ = YYTextLine.lineWith(ctLine: ctTruncatedLine, position: lastLine.position, vertical: isVerticalForm)
        ...
        if (isVerticalForm) {
            let rotateCharset = YYTextUtilities.textVerticalFormRotateCharacterSet
            let rotateMoveCharset = YYTextUtilities.textVerticalFormRotateAndMoveCharacterSet
            let lineBlock: ((YYTextLine?) -> Void) = { line in
                ...
                line!.verticalRotateRange = [[YYTextRunGlyphRange]]()
                for i in 0..<runCount {
                    let run = unsafeBitCast(CFArrayGetValueAtIndex(runs, i), to: CTRun.self)
                    var runRanges = [YYTextRunGlyphRange]()
                    ...
                    let isColorGlyph: Bool = YYTextUtilities.textCTFontContainsColorBitmapGlyphs(font)
                    var prevIdx: Int = 0
                    var prevMode = YYTextRunGlyphDrawMode.horizontal
                    ...
                    for g in 0..<glyphCount {
                        ...
                        let mode = glyphRotateMove ? YYTextRunGlyphDrawMode.verticalRotateMove : (glyphRotate ? YYTextRunGlyphDrawMode.verticalRotate : YYTextRunGlyphDrawMode.horizontal);
                        if g == 0 {
                            prevMode = mode
                        } else if mode != prevMode {
                            let aRange = YYTextRunGlyphRange.range(with: NSRange(location: prevIdx, length: g - prevIdx), drawMode: prevMode)
                    ...
                    if prevIdx < glyphCount {
                        let aRange = YYTextRunGlyphRange.range(with: NSRange(location: prevIdx, length: glyphCount - prevIdx), drawMode: prevMode)
                        runRanges.append(aRange)
                    }
        ...
        if visibleRange_.length > 0 {
            ...
            let block: ((_ attrs: [AnyHashable : Any]?, _ range: NSRange, _ stop: UnsafeMutablePointer<ObjCBool>?) -> Void)? = { attrs, range, stop in
                if attrs?[YYTextAttribute.textHighlightAttributeName] != nil {
                    self.containsHighlight = true
                }
                if attrs?[YYTextAttribute.textBlockBorderAttributeName] != nil {
                    self.needDrawBlockBorder = true
                }
                if attrs?[YYTextAttribute.textBackgroundBorderAttributeName] != nil {
                    self.needDrawBackgroundBorder = true
                }
                if attrs?[YYTextAttribute.textShadowAttributeName] != nil || attrs?[NSAttributedString.Key.shadow] != nil {
                    self.needDrawShadow = true
                }
                if attrs?[YYTextAttribute.textUnderlineAttributeName] != nil {
                    self.needDrawUnderline = true
                }
                if attrs?[YYTextAttribute.textAttachmentAttributeName] != nil {
                    self.needDrawAttachment = true
                }
                if attrs?[YYTextAttribute.textInnerShadowAttributeName] != nil {
                    self.needDrawInnerShadow = true
                }
                if attrs?[YYTextAttribute.textStrikethroughAttributeName] != nil {
                    self.needDrawStrikethrough = true
                }
                if attrs?[YYTextAttribute.textBorderAttributeName] != nil {
    ...
    public class func layout(with containers: [YYTextContainer]?, text: NSAttributedString?) -> [YYTextLayout]? {
    ...
    public class func layout(with containers: [YYTextContainer]?, text: NSAttributedString?, range: NSRange) -> [YYTextLayout]? {
        ...
        for i in 0..<maxCount {
            ...
            guard let layout = YYTextLayout.init(container: container, text: text, range: range) else {
                return nil
            }
    ...
    public func encode(with aCoder: NSCoder) {
        ...
        if let aText = text {
            textData = YYTextArchiver.archivedData(withRootObject: aText)
        }
    ...
    required convenience public init?(coder aDecoder: NSCoder) {
        ...
        let container = aDecoder.decodeObject(forKey: "container") as? YYTextContainer
     ...
     */
    private func _run(for line: YYTextLine?, position: YYTextPosition?) -> CTRun? {
        ...
        while i < max {
            ...
            if position?.affinity == YYTextAffinity.backward {
     ...
     */
    @discardableResult
    private func _insideComposedCharacterSequences(_ line: YYTextLine?, position: Int, block: @escaping (_ `left`: CGFloat, _ `right`: CGFloat, _ prev: Int, _ next: Int) -> Void) -> Bool {
     ...
     */
    @discardableResult
    private func _insideEmoji(_ line: YYTextLine?, position: Int, block: @escaping (_ `left`: CGFloat, _ `right`: CGFloat, _ prev: Int, _ next: Int) -> Void) -> Bool {
        ...
        for r in 0..<rMax {
            ...
            if !YYTextUtilities.textCTFontContainsColorBitmapGlyphs(font) {
                continue
            }
    ...
    /**
     ...
     @return YES if RTL.
     */
    private func _isRightToLeft(in line: YYTextLine?, at point: CGPoint) -> Bool {
        if line == nil {
            return false
        }
        // get write direction
     ...
     */
    private func _correctedRange(withEdge range: YYTextRange) -> YYTextRange? {
        var range = range
        let visibleRange = self.visibleRange
        var start = range.start
        var end = range.end
        if start.offset == visibleRange.location && start.affinity == YYTextAffinity.backward {
            start = YYTextPosition.position(with: start.offset, affinity: YYTextAffinity.forward)
        }
        if end.offset == visibleRange.location + visibleRange.length && start.affinity == YYTextAffinity.forward {
            end = YYTextPosition.position(with: end.offset, affinity: YYTextAffinity.backward)
        }
        if start != range.start || end != range.end {
            range = YYTextRange.range(with: start, end: end)
        }
        return range
    }
     ...
     */
    @objc(offsetForYYTextPosition:lineIndex:)
    ...
    public func closestPosition(to point: CGPoint) -> YYTextPosition? {
        ...
        var line: YYTextLine? = lines[lineIndex]
        ...
        if position <= visibleRange.location {
            return YYTextPosition.position(with: visibleRange.location, affinity: YYTextAffinity.forward)
        } else if position >= visibleRange.location + visibleRange.length {
            return YYTextPosition.position(with: visibleRange.location + visibleRange.length, affinity: YYTextAffinity.backward)
        }
        var finalAffinity = YYTextAffinity.forward
        var finalAffinityDetected = false
        // binding range
        var bindingRange = NSRange(location: 0, length: 0)
        let binding = text!.attribute(NSAttributedString.Key(rawValue: YYTextAttribute.textBindingAttributeName), at: position, longestEffectiveRange: &bindingRange, in: NSRange(location: 0, length: text!.length))
        
        if let _ = binding, bindingRange.length > 0 {
            let headLineIdx: Int = self.lineIndex(for: YYTextPosition.position(with: bindingRange.location))
            let tailLineIdx: Int = self.lineIndex(for: YYTextPosition.position(with: bindingRange.location + bindingRange.length, affinity: YYTextAffinity.backward))
                ...
                if `left` != CGFloat.greatestFiniteMagnitude && `right` != CGFloat.greatestFiniteMagnitude {
                    if container.isVerticalForm {
                        if abs(Float(point.y - `left`)) < abs(Float(point.y - `right`)) {
                            ...
                            finalAffinity = YYTextAffinity.forward
                        } else {
                            position = bindingRange.location + bindingRange.length
                            finalAffinity = YYTextAffinity.backward
                        }
                    } else {
        ...
        if line!.range.length == 0 {
            ...
            return YYTextPosition.position(with: line!.range.location, affinity: behind ? YYTextAffinity.backward : YYTextAffinity.forward)
        }
        ...
        if line!.range.length <= 2 {
            ...
            if YYTextUtilities.textIsLinebreakString(str) {
                // an empty line ("\r", "\n", "\r\n")
                return YYTextPosition.position(with: line!.range.location)
            }
        }
        ...
        if position >= line!.range.location + line!.range.length - 1 {
            if position > line!.range.location {
                ...
                if YYTextUtilities.textIsLinebreakChar(c1) {
                    position -= 1
                    if position > line!.range.location {
                        let c0 = (text!.string as NSString).character(at: position - 1)
                        if YYTextUtilities.textIsLinebreakChar(c0) {
                            position -= 1
                        }
                    }
                }
        ...
        if position == line!.range.location {
            return YYTextPosition.position(with: position)
        }
        if position == line!.range.location + line!.range.length {
            return YYTextPosition.position(with: position, affinity: YYTextAffinity.backward)
        }
        ...
        if !finalAffinityDetected {
            ...
            if ofs != CGFloat.greatestFiniteMagnitude {
                ...
                if position >= line!.range.location + line!.range.length {
                    finalAffinity = RTL ? YYTextAffinity.forward : YYTextAffinity.backward
                } else if position <= line!.range.location {
                    finalAffinity = RTL ? YYTextAffinity.backward : YYTextAffinity.forward
                } else {
                    finalAffinity = (ofs < (isVertical ? point.y : point.x) && !RTL) ? YYTextAffinity.forward : YYTextAffinity.backward
                }
            }
        }
        return YYTextPosition.position(with: position, affinity: finalAffinity)
    }
    ...
    public func position(for point: CGPoint, oldPosition: YYTextPosition?, otherPosition: YYTextPosition?) -> YYTextPosition? {
        ...
        if container.isVerticalForm {
            if old.compare(other) == .orderedAscending {
                // search backward
                let range: YYTextRange? = textRange(byExtending: otherPosition, in: UITextLayoutDirection.up, offset: 1)
                if range != nil {
                    return range?.start
                }
            } else {
                // search forward
                let range: YYTextRange? = textRange(byExtending: otherPosition, in: UITextLayoutDirection.down, offset: 1)
                if range != nil {
                    return range?.end
                }
            }
            ...
            if old.compare(other) == .orderedAscending {
                // search backward
                let range: YYTextRange? = textRange(byExtending: otherPosition, in: UITextLayoutDirection.left, offset: 1)
                if range != nil {
                    return range?.start
                }
            } else {
                // search forward
                let range: YYTextRange? = textRange(byExtending: otherPosition, in: UITextLayoutDirection.right, offset: 1)
                if range != nil {
                    return range?.end
                }
            }
    ...
    public func textRange(at point: CGPoint) -> YYTextRange? {
        ...
        if container.isVerticalForm {
            let range: YYTextRange? = textRange(byExtending: pos, in: ((rect.origin.y ) >= point.y && !RTL) ? .up : .down, offset: 1)
            return range
        } else {
            let range: YYTextRange? = textRange(byExtending: pos, in: ((rect.origin.x ) >= point.x && !RTL) ? .left : .right, offset: 1)
            return range
        }
    }
    ...
    public func closestTextRange(at point: CGPoint) -> YYTextRange? {
        ...
        let range: YYTextRange? = textRange(byExtending: pos, in: direction, offset: 1)
        return range
    }
    ...
    public func textRange(byExtending position: YYTextPosition?) -> YYTextRange? {
        ...
        if p.offset == visibleStart {
            return YYTextRange.range(with: NSRange(location: p.offset, length: 0))
        } else if p.offset == visibleEnd {
            return YYTextRange.range(with: NSRange(location: p.offset, length: 0), affinity: YYTextAffinity.backward)
        }
        
        // binding range
        var tRange = NSRange(location: 0, length: 0)
        let binding = text!.attribute(NSAttributedString.Key(rawValue: YYTextAttribute.textBindingAttributeName), at: p.offset, longestEffectiveRange: &tRange, in: visibleRange)
        if binding != nil && tRange.length != 0 && tRange.location < p.offset {
            return YYTextRange.range(with: tRange)
        }
        ...
        if lineIndex != NSNotFound {
            ...
            if emoji || seq {
                return YYTextRange.range(with: NSRange(location: _prev, length:_next - _prev))
            }
        }
        ...
        if p.offset > visibleStart && p.offset < visibleEnd {
            ...
            if (c0 == ("\r" as NSString).character(at: 0)) && p.offset < visibleEnd {
                ...
                if c1 == ("\n" as NSString).character(at: 0) {
                    return YYTextRange.range(with: YYTextPosition.position(with: p.offset - 1), end: YYTextPosition.position(with: p.offset + 1))
                }
            }
            if YYTextUtilities.textIsLinebreakChar(c0) && p.affinity == YYTextAffinity.backward {
                let str = (text!.string as NSString).substring(to: p.offset)
                let len: Int = YYTextUtilities.textLinebreakTailLength(str)
                return YYTextRange.range(with: YYTextPosition.position(with: p.offset - len), end: YYTextPosition.position(with: p.offset))
            }
        }
        
        return YYTextRange.range(with: NSRange(location: p.offset, length: 0), affinity: p.affinity)
    }
    ...
    public func textRange(byExtending position: YYTextPosition?, in direction: UITextLayoutDirection, offset: Int) -> YYTextRange? {
        ...
        if !forwardMove && p.offset == visibleStart {
            return YYTextRange.range(with: NSRange(location: visibleRange.location, length: 0))
        ...
        let allForward = YYTextRange.range(with: fromRange.start, end: YYTextPosition.position(with: visibleEnd))
        let allBackward = YYTextRange.range(with: YYTextPosition.position(with: visibleStart), end: fromRange.end)
        ...
        if (verticalMove) { // up/down in text layout
            ...
            var mostLeftLine = YYTextLine()
            var mostRightLine = YYTextLine()
            ...
            if afinityEdge {
                if pos == insideLine.range.location + insideLine.range.length {
                    ...
                    let lineBreakLen: Int = YYTextUtilities.textLinebreakTailLength(subStr)
                    extPos = YYTextPosition.position(with: pos - lineBreakLen)
                } else {
                    extPos = YYTextPosition.position(with: pos)
                }
            } else {
                extPos = YYTextPosition.position(with: pos)
            }
            ...
            if forwardMove {
                return YYTextRange.range(with: fromRange.start, end: ext.end)
            } else {
                return YYTextRange.range(with: ext.start, end: fromRange.end)
            }
        } else {
            let toPosition = YYTextPosition.position(with: p.offset + (forwardMove ? offset : -offset))
            ...
            return YYTextRange.range(with: NSRange(location: start, length: end - start))
        }
    }
    ...
    public func lineIndex(for position: YYTextPosition?) -> Int {
    ...
    public func linePosition(for position: YYTextPosition?) -> CGPoint {
    ...
    public func caretRect(for position: YYTextPosition) -> CGRect {
        let lineIndex = self.lineIndex(for: position)
        if lineIndex == NSNotFound {
            return CGRect.null
        }
    ...
    public func firstRect(for range: YYTextRange) -> CGRect {
        var range = range
     ...
     */
    @objc(rectForRange:)
    public func rect(for range: YYTextRange?) -> CGRect {
        var rects: [YYTextSelectionRect]? = nil
        if let aRange = range {
    ...
    public func selectionRects(for range: YYTextRange) -> [YYTextSelectionRect] {
        
        let range = _correctedRange(withEdge: range)!
        let isVertical = container.isVerticalForm
        var rects: [YYTextSelectionRect] = []
        ...
        if startLineIndex > endLineIndex {
            YYTextUtilities.numberSwap(&startLineIndex, b: &endLineIndex)
        }
        ...
        if startLine.row == endLine.row {
            ...
            if offsetStart > offsetEnd {
                YYTextUtilities.numberSwap(&offsetStart, b: &offsetEnd)
            }
            let rect = YYTextSelectionRect()
            if isVertical {
            ...
            let topRect = YYTextSelectionRect()
            ...
            let bottomRect = YYTextSelectionRect()
    ...
    public func selectionRectsWithoutStartAndEnd(for range: YYTextRange) -> [YYTextSelectionRect] {
    ...
    public func selectionRectsWithOnlyStartAndEnd(for range: YYTextRange) -> [YYTextSelectionRect] {
    ...
    public func draw(in context: CGContext?, size: CGSize, point: CGPoint, view: UIView?, layer: CALayer?, debug: YYTextDebugOption?, cancel: (() -> Bool)? = nil) {
        
        if needDrawBlockBorder, let c = context {
            if let _cancel = cancel, _cancel() { return }
            YYTextDrawBlockBorder(self, context: c, size: size, point: point, cancel: cancel)
        }
        
        if needDrawBackgroundBorder, let c = context {
            if let _cancel = cancel, _cancel() { return }
            YYTextDrawBorder(self, context: c, size: size, point: point, type: YYTextBorderType.backgound, cancel: cancel)
        }
        
        if needDrawShadow, let c = context {
            if let _cancel = cancel, _cancel() { return }
            YYTextDrawShadow(self, context: c, size: size, point: point, cancel: cancel)
        }
        
        if needDrawUnderline, let c = context {
            if let _cancel = cancel, _cancel() { return }
            YYTextDrawDecoration(self, context: c, size: size, point: point, type: YYTextDecorationType.underline, cancel: cancel)
        }
        
        if needDrawText, let c = context {
            if let _cancel = cancel, _cancel() { return }
            YYTextDrawText(self, context: c, size: size, point: point, cancel: cancel)
        }
        
        if needDrawAttachment && (context != nil || view != nil || layer != nil) {
            if let _cancel = cancel, _cancel() { return }
            YYTextDrawAttachment(self, context: context, size: size, point: point, targetView: view, targetLayer: layer, cancel: cancel)
        }
        
        if needDrawInnerShadow, let c = context {
            if let _cancel = cancel, _cancel() { return }
            YYTextDrawInnerShadow(self, context: c, size: size, point: point, cancel: cancel)
        }
        
        if needDrawStrikethrough, let c = context {
            if let _cancel = cancel, _cancel() { return }
            YYTextDrawDecoration(self, context: c, size: size, point: point, type: YYTextDecorationType.strikethrough, cancel: cancel)
        }
        
        if needDrawBorder, let c = context {
            if let _cancel = cancel, _cancel() { return }
            YYTextDrawBorder(self, context: c, size: size, point: point, type: YYTextBorderType.normal, cancel: cancel)
        }
        
        if let d = debug?.needDrawDebug, d, let c = context {
            if let _cancel = cancel, _cancel() { return }
            YYTextDrawDebug(self, context: c, size: size, point: point, op: debug)
        }
    }
    ...
    public func draw(in context: CGContext?, size: CGSize, debug: YYTextDebugOption?) {
...
fileprivate struct YYTextDecorationType : OptionSet {
    let rawValue: Int
    static let underline = YYTextDecorationType(rawValue: 1 << 0)
    static let strikethrough = YYTextDecorationType(rawValue: 1 << 1)
}

fileprivate struct YYTextBorderType : OptionSet {
    let rawValue: Int
    static let backgound = YYTextBorderType(rawValue: 1 << 0)
    static let normal = YYTextBorderType(rawValue: 1 << 1)
}

private func YYTextMergeRectInSameLine(rect1: CGRect, rect2: CGRect, isVertical: Bool) -> CGRect {
    if isVertical {
...
private func YYTextGetRunsMaxMetric(runs: CFArray, xHeight: UnsafeMutablePointer<CGFloat>, underlinePosition: UnsafeMutablePointer<CGFloat>?, lineThickness: UnsafeMutablePointer<CGFloat>) {
    let xHeight = xHeight
...
private func YYTextDrawRun(line: YYTextLine, run: CTRun, context: CGContext, size: CGSize, isVertical: Bool, runRanges: [YYTextRunGlyphRange]?, verticalOffset: CGFloat) {
    
    let runTextMatrix: CGAffineTransform = CTRunGetTextMatrix(run)
    let runTextMatrixIsID = runTextMatrix.isIdentity
    let runAttrs = CTRunGetAttributes(run) as! [String: AnyObject]
    
    let glyphTransformValue = runAttrs[YYTextAttribute.textGlyphTransformAttributeName] as? NSValue
        ...
        do {
            ...
            if isVertical {
                ...
                for oneRange in runRanges ?? [] {
                    ...
                    let mode: YYTextRunGlyphDrawMode = oneRange.drawMode
                    ...
                    for g in range.location..<rangeMax {
                        ...
                        do {
                            ...
                            if mode != .horizontal {
                                ...
                                if mode == YYTextRunGlyphDrawMode.verticalRotateMove {
                                    x += w
                                    y += w
                                }
                                context.textPosition = CGPoint(x: x, y: y)
                            } else {
                                context.rotate(by: YYTextUtilities.textRadians(from: (-90)))
                            ...
                            if YYTextUtilities.textCTFontContainsColorBitmapGlyphs((runFont)) {
...
private func YYTextSetLinePatternInContext(style: YYTextLineStyle, width: CGFloat, phase: CGFloat, context: CGContext) {
    ...
    if pattern == YYTextLineStyle.none.rawValue {
        // YYTextLineStylePatternSolid
        context.setLineDash(phase: phase, lengths: [])
    } else if pattern == YYTextLineStyle.patternDot.rawValue {
        let lengths = [width * dot, width * space]
        context.setLineDash(phase: phase, lengths: lengths)
    } else if pattern == YYTextLineStyle.patternDash.rawValue {
        let lengths = [width * dash, width * space]
        context.setLineDash(phase: phase, lengths: lengths)
    } else if pattern == YYTextLineStyle.patternDashDot.rawValue {
        let lengths = [width * dash, width * space, width * dot, width * space]
        context.setLineDash(phase: phase, lengths: lengths)
    } else if pattern == YYTextLineStyle.patternDashDotDot.rawValue {
        let lengths = [width * dash, width * space, width * dot, width * space, width * dot, width * space]
        context.setLineDash(phase: phase, lengths: lengths)
    } else if pattern == YYTextLineStyle.patternCircleDot.rawValue {
...
private func YYYYTextDrawBorderRects(context: CGContext, size: CGSize, border: YYTextBorder, rects: [NSValue], isVertical: Bool) {
    ...
    if border.strokeColor != nil && border.lineStyle.rawValue > 0 && border.strokeWidth > 0 {
        ...
        YYTextSetLinePatternInContext(style: border.lineStyle, width: border.strokeWidth, phase: 0, context: context)
        var inset: CGFloat = -border.strokeWidth * 0.5
        if (border.lineStyle.rawValue & 0xff) == YYTextLineStyle.thick.rawValue {
            inset *= 2
        ...
        //------------------------- second line ------------------------------//
        if (border.lineStyle.rawValue & 0xff) == YYTextLineStyle.double.rawValue {
...
private func YYTextDrawLineStyle(context: CGContext, length: CGFloat, lineWidth: CGFloat, style: YYTextLineStyle, position: CGPoint, color: CGColor, isVertical: Bool) {
    ...
    do {
        if isVertical {
            ...
            var y2: CGFloat
            var w: CGFloat
            y1 = YYTextUtilities.textCGFloat(pixelRound: position.y)
            y2 = YYTextUtilities.textCGFloat(pixelRound: (position.y + length))
            w = styleBase == YYTextLineStyle.thick.rawValue ? lineWidth * 2 : lineWidth
            let linePixel = YYTextUtilities.textCGFloat(toPixel: w)
            if abs(Float(linePixel - floor(linePixel))) < 0.1 {
                let iPixel = Int(linePixel)
                if iPixel == 0 || (iPixel % 2) != 0 {
                    // odd line pixel
                    x = YYTextUtilities.textCGFloat(pixelHalf: position.x)
                } else {
                    x = YYTextUtilities.textCGFloat(pixelFloor: position.x)
                }
            } else {
                x = position.x
            }
            ...
            YYTextSetLinePatternInContext(style: style, width: lineWidth, phase: position.y, context: context)
            context.setLineWidth(w)
            if styleBase == YYTextLineStyle.single.rawValue {
                context.move(to: CGPoint(x: x, y: y1))
                context.addLine(to: CGPoint(x: x, y: y2))
                context.strokePath()
            } else if styleBase == YYTextLineStyle.thick.rawValue {
                context.move(to: CGPoint(x: x, y: y1))
                context.addLine(to: CGPoint(x: x, y: y2))
                context.strokePath()
            } else if styleBase == YYTextLineStyle.double.rawValue {
            ...
            var y: CGFloat = 0
            var w: CGFloat = 0
            x1 = YYTextUtilities.textCGFloat(pixelRound: position.x)
            x2 = YYTextUtilities.textCGFloat(pixelRound: position.x + length)
            w = styleBase == YYTextLineStyle.thick.rawValue ? lineWidth * 2 : lineWidth
            let linePixel = YYTextUtilities.textCGFloat(toPixel: w)
            if abs(Float(linePixel - floor(linePixel))) < 0.1 {
                let iPixel = Int(linePixel)
                if iPixel == 0 || (iPixel % 2) != 0 {
                    // odd line pixel
                    y = YYTextUtilities.textCGFloat(pixelHalf: position.y)
                } else {
                    y = YYTextUtilities.textCGFloat(pixelFloor: position.y)
                }
            } else {
                y = position.y
            }
            context.setStrokeColor(color)
            YYTextSetLinePatternInContext(style: style, width: lineWidth, phase: position.x, context: context)
            context.setLineWidth(w)
            if styleBase == YYTextLineStyle.single.rawValue {
...
private func YYTextDrawText(_ layout: YYTextLayout, context: CGContext, size: CGSize, point: CGPoint, cancel: (() -> Bool)? = nil) {
    context.saveGState()
    do {
        ...
        for l in lines {
            ...
            for r in 0..<rMax {
                ...
                YYTextDrawRun(line: line, run: run, context: context, size: size, isVertical: isVertical, runRanges: lineRunRanges?[r], verticalOffset: verticalOffset)
            }
...
private func YYTextDrawBlockBorder(_ layout: YYTextLayout, context: CGContext, size: CGSize, point: CGPoint, cancel: (() -> Bool)? = nil) {
    ...
    while l < lMax {
        ...
        for r in 0..<rMax {
            ...
            guard let border = attrs?[YYTextAttribute.textBlockBorderAttributeName] as? YYTextBorder else {
                continue
            }
            ...
            repeat {
                ...
                if next.row != lineContinueRow {
                    let nextBorder = layout.text?.yy_attribute(NSAttributedString.Key(rawValue: YYTextAttribute.textBlockBorderAttributeName), at: next.range.location) as? YYTextBorder
            ...
            YYYYTextDrawBorderRects(context: context, size: size, border: border, rects: [NSValue(cgRect: unionRect)], isVertical: isVertical)
            l = lineContinueIndex
            break
        }
        l += 1
...
private func YYTextDrawBorder(_ layout: YYTextLayout, context: CGContext, size: CGSize, point: CGPoint, type: YYTextBorderType, cancel: (() -> Bool)? = nil) {
    ...
    let verticalOffset: CGFloat = isVertical ? (size.width - layout.container.size.width) : 0
    let lines = layout.lines
    let borderKey = (type == YYTextBorderType.normal ? YYTextAttribute.textBorderAttributeName : YYTextAttribute.textBackgroundBorderAttributeName)
    ...
    while l < lMax {
        ...
        while r < rMax {
            ...
            guard let border = attrs?[borderKey] as? YYTextBorder else {
                r += 1
                continue
            }
            ...
            for ll in l..<lMax {
                ...
                for rr in rr_..<rrMax {
                    ...
                    let iBorder = iAttrs?[borderKey] as? YYTextBorder
                    if !(border == iBorder) {
                        endFound = true
                        break
                    }
                    ...
                    if isVertical {
                        YYTextUtilities.numberSwap(&iRunPosition.x, b: &iRunPosition.y)
            ...
            for re in 0..<reMax {
                ...
                if isVertical {
                    if abs(Float((rect.origin.x) - (curRect.origin.x))) < 1 {
                        curRect = YYTextMergeRectInSameLine(rect1: rect, rect2: curRect, isVertical: isVertical)
                    } else {
            ...
            YYYYTextDrawBorderRects(context: context, size: size, border: border, rects: drawRects, isVertical: isVertical)
            if l == endLineIndex {
                r = endRunIndex
            } else {
                l = endLineIndex - 1
...
private func YYTextDrawDecoration(_ layout: YYTextLayout, context: CGContext, size: CGSize, point: CGPoint, type: YYTextDecorationType, cancel: (() -> Bool)? = nil) {
    ...
    for l in 0..<lMax {
        ...
        for r in 0..<rMax {
            ...
            let attrs = CTRunGetAttributes(run) as? [AnyHashable : Any]
            let underline = attrs?[YYTextAttribute.textUnderlineAttributeName] as? YYTextDecoration
            let strikethrough = attrs?[YYTextAttribute.textStrikethroughAttributeName] as? YYTextDecoration
            var needDrawUnderline = false
            var needDrawStrikethrough = false
            if (type.rawValue & YYTextDecorationType.underline.rawValue) != 0 && underline?.style.rawValue ?? 0 > 0 {
                needDrawUnderline = true
            }
            if (type.rawValue & YYTextDecorationType.strikethrough.rawValue) != 0 && strikethrough?.style.rawValue ?? 0 > 0 {
            ...
            if YYTextUtilities.textIsLinebreakString((runStr)) {
                continue // may need more checks...
            }
            var xHeight: CGFloat = 0
            var underlinePosition: CGFloat = 0
            var lineThickness: CGFloat = 0
            YYTextGetRunsMaxMetric(runs: runs, xHeight: &xHeight, underlinePosition: &underlinePosition, lineThickness: &lineThickness)
                ...
                while shadow != nil {
                    ...
                    do {
                        ...
                        do {
                            ...
                            YYTextDrawLineStyle(context: context, length: length, lineWidth: thickness, style: underline!.style, position: underlineStart, color: color!, isVertical: isVertical)
                        }
                        context.restoreGState()
                    }
                    context.restoreGState()
            ...
            if needDrawStrikethrough {
                ...
                YYTextDrawLineStyle(context: context, length: length, lineWidth: thickness, style: strikethrough!.style, position: strikethroughStart, color: color!, isVertical: isVertical)
            }
        }
    }
    context.restoreGState()
...
private func YYTextDrawAttachment(_ layout: YYTextLayout, context: CGContext?, size: CGSize, point: CGPoint, targetView: UIView?, targetLayer: CALayer?, cancel: (() -> Bool)? = nil) {
    ...
    for i in 0..<max {
        ...
        rect = YYTextUtilities.textCGRectFit(with: a.contentMode, rect: rect!, size: asize)
        rect = YYTextUtilities.textCGRect(pixelRound: rect!)
        rect = rect?.standardized
...
private func YYTextDrawShadow(_ layout: YYTextLayout, context: CGContext, size: CGSize, point: CGPoint, cancel: (() -> Bool)? = nil) {
    ...
    do {
        ...
        for l in 0..<lMax {
            ...
            for r in 0..<rMax {
                ...
                let attrs = CTRunGetAttributes(run) as? [AnyHashable : Any]
                var shadow = attrs?[YYTextAttribute.textShadowAttributeName] as? YYTextShadow
                let nsShadow = YYTextShadow.shadow(with: (attrs?[NSAttributedString.Key.shadow] as? NSShadow)) // NSShadow compatible
...
private func YYTextDrawInnerShadow(_ layout: YYTextLayout, context: CGContext, size: CGSize, point: CGPoint, cancel: (() -> Bool)? = nil) {
    ...
    for l in 0..<lMax {
        ...
        for r in 0..<rMax {
            ...
            context.textPosition = CGPoint(x: linePosX, y: linePosY)
            let attrs = CTRunGetAttributes(run) as? [AnyHashable : Any]
            var shadow = attrs?[YYTextAttribute.textInnerShadowAttributeName] as? YYTextShadow
            while shadow != nil {
                if shadow?.color == nil {
                    shadow = shadow?.subShadow
                    continue
                ...
                if let _ = runAttrs[YYTextAttribute.textGlyphTransformAttributeName] as? NSValue {
...
private func YYTextDrawDebug(_ layout: YYTextLayout, context: CGContext, size: CGSize, point: CGPoint, op: YYTextDebugOption?) {
    
    UIGraphicsPushContext(context)
    context.saveGState()
    context.translateBy(x: point.x, y: point.y)
    context.setLineWidth(1.0 / YYTextUtilities.textScreenScale)
    ...
    if op?.ctFrameBorderColor != nil || op?.ctFrameFillColor != nil {
        ...
        if path == nil {
            ...
            if op?.ctFrameBorderColor != nil {
                rect = YYTextUtilities.textCGRect(pixelHalf: rect)
            } else {
                rect = YYTextUtilities.textCGRect(pixelRound: rect)
            }
    ...
    for l in 0..<lMax {
        ...
        if op?.ctLineFillColor != nil {
            ...
            context.addRect(YYTextUtilities.textCGRect(pixelRound: lineBounds))
            context.fillPath()
        }
        if op?.ctLineBorderColor != nil {
            op!.ctLineBorderColor!.setStroke()
            context.addRect(YYTextUtilities.textCGRect(pixelHalf: lineBounds))
            context.strokePath()
        }
        if op?.baselineColor != nil {
            op!.baselineColor!.setStroke()
            if isVertical {
                let x: CGFloat = YYTextUtilities.textCGFloat(pixelHalf: line.position.x)
                let y1: CGFloat = YYTextUtilities.textCGFloat(pixelHalf: line.top)
                let y2: CGFloat = YYTextUtilities.textCGFloat(pixelHalf: line.bottom)
                context.move(to: CGPoint(x: x, y: y1))
                context.addLine(to: CGPoint(x: x, y: y2))
                context.strokePath()
            } else {
                let x1: CGFloat = YYTextUtilities.textCGFloat(pixelHalf: lineBounds.origin.x)
                let x2: CGFloat = YYTextUtilities.textCGFloat(pixelHalf: (lineBounds.origin.x + lineBounds.size.width))
                let y: CGFloat = YYTextUtilities.textCGFloat(pixelHalf: line.position.y)
                context.move(to: CGPoint(x: x1, y: y))
                context.addLine(to: CGPoint(x: x2, y: y))
                context.strokePath()
            }
        ...
        if op?.ctLineNumberColor != nil {
            ...
            num.yy_font = UIFont.systemFont(ofSize: 6)
        ...
        if op?.ctRunFillColor != nil || op?.ctRunBorderColor != nil || op?.ctRunNumberColor != nil || op?.cgGlyphFillColor != nil || op?.cgGlyphBorderColor != nil {
            ...
            for r in 0..<rMax {
                ...
                if isVertical {
                    YYTextUtilities.numberSwap(&runPosition.x, b: &runPosition.y)
                ...
                if op?.ctRunFillColor != nil {
                    ...
                    context.addRect(YYTextUtilities.textCGRect(pixelRound: runTypoBounds))
                    context.fillPath()
                }
                if op?.ctRunBorderColor != nil {
                    op!.ctRunBorderColor!.setStroke()
                    context.addRect(YYTextUtilities.textCGRect(pixelHalf: runTypoBounds))
                    context.strokePath()
                }
                ...
                if op?.cgGlyphBorderColor != nil || op?.cgGlyphFillColor != nil {
                    for g in 0..<glyphCount {
                        ...
                        var rect = CGRect.zero
                        if isVertical {
                            YYTextUtilities.numberSwap(&pos.x, b: &pos.y)
                            pos.x = runPosition.x
                        ...
                        if op?.cgGlyphFillColor != nil {
                            ...
                            context.addRect(YYTextUtilities.textCGRect(pixelRound: rect))
                            context.fillPath()
                        }
                        if op?.cgGlyphBorderColor != nil {
                            op!.cgGlyphBorderColor!.setStroke()
                            context.addRect(YYTextUtilities.textCGRect(pixelHalf: rect))
                            context.strokePath()
                        }
                    }
                }
