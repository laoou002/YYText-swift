 ...
 */
public class YYTextArchiver: NSKeyedArchiver, NSKeyedArchiverDelegate {
    ...
    override public class func archivedData(withRootObject rootObject: Any) -> Data {
        ...
        if #available(iOS 11.0, *) {
            let archiver = YYTextArchiver(requiringSecureCoding: false)
            archiver.encodeRootObject(rootObject)

            data = archiver.encodedData
        } else {
            let d = NSMutableData()
            let archiver = YYTextArchiver(forWritingWith: d)
            archiver.encodeRootObject(rootObject)
    ...
    public func archiver(_ archiver: NSKeyedArchiver, willEncode object: Any) -> Any? {
        ...
        if typeID == CTRunDelegateGetTypeID() {
            ...
            // 错误示例：unsafeBitCast(ref, to: YYTextRunDelegate.self) 会导致 Crash
//            let p = ref.assumingMemoryBound(to: YYTextRunDelegate.self)
//            return p.pointee
            return ref.load(as: YYTextRunDelegate.self)
            
        } else if typeID == CTRubyAnnotationGetTypeID() {
            
            let ctRuby = object as! CTRubyAnnotation
            let ruby = YYTextRubyAnnotation.ruby(with: ctRuby)
...
public class TextUnarchiver: NSKeyedUnarchiver, NSKeyedUnarchiverDelegate {
    ...
    public func unarchiver(_ unarchiver: NSKeyedUnarchiver, didDecode object: Any?) -> Any? {
        ...
        if type(of: obj) == YYTextRunDelegate.self {
            
            let runDelegate = obj as! YYTextRunDelegate
        ...
        } else if type(of: obj) == YYTextRubyAnnotation.self {
            
            let ruby = object as! YYTextRubyAnnotation
