 ...
 */
public class YYTextLine: NSObject {
    ...
    /*/< Run rotate range */
    @objc public var verticalRotateRange: [[YYTextRunGlyphRange]]?
    ...
    public class func lineWith(ctLine: CTLine, position: CGPoint, vertical isVertical: Bool) -> YYTextLine {
    ...
    private func reloadBounds() {
        ...
        var attachments_ = [YYTextAttachment]()
        ...
        for r in 0..<runCount {
            ...
            if let attachment = attrs?[YYTextAttribute.textAttachmentAttributeName] as? YYTextAttachment {
    ...
    public override var description: String {
        ...
        desc += String(format: "<YYTextLine: %p> row: %zd range: %tu, %tu", self, row, range.location, range.length)
...
@objc public enum YYTextRunGlyphDrawMode : Int {
    /// No rotate.
    case horizontal = 0
...
/**
 A range in CTRun, used for vertical form.
 */
public class YYTextRunGlyphRange: NSObject {
    
    @objc public var glyphRangeInRun = NSRange(location: 0, length: 0)
    @objc public var drawMode = YYTextRunGlyphDrawMode.horizontal
    
    @objc(rangeWithRange:drawMode:)
    public class func range(with range: NSRange, drawMode mode: YYTextRunGlyphDrawMode) -> YYTextRunGlyphRange {
        
        let one = YYTextRunGlyphRange()
